...
Using TDD with legacy code --code that does have a lot of (unit) tests, and that was never designed for testability--
is harder the starting TDD in a green field. Still, there are options: at least make all new code testable and use the
concepts of TDD where possible.
|BR|
In one of the upcoming blogs, I will give some tricks on that.


Typically, people advise to start with TDD, before applying BDD. For legacy, the opposite is often true. As BDD depends
less on the style of coding, legacy (style) code isn’t a game-changer.
|BR|
As long as one has a sound set of acceptance-test, preferably ones that can be automated, that is s great start-point:
just run those tests (automatically) every few days, of every pull request, or so.

And, design more ATSes. Add a set for all new features, user story, or whatever you call your requirements.
|BR|
Again, the ideas are starting soon. Don’t wait on the code. You don’t even have to wait on the final story. Start as
...
So, make sure all omissions are removed before the SW-engineers write the code – else they writes te wrong code!
